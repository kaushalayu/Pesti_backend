...
exports.getMyInventory = catchAsync(async (req, res, next) => {
  ...
  for (const dist of distributions) {
    for (const item of dist.items) {
      ...
      inventoryMap[chemId].currentStock += (item.quantity - (item.returnedQty || 0));
    }
  }
  
  const inventory = Object.values(inventoryMap).filter(item => item.currentStock > 0);
...
exports.useStock = catchAsync(async (req, res, next) => {
  ...
  const itemIndex = distribution.items.findIndex(i => i.chemicalId.toString() === chemicalId);
  ...
  if (usedQty > availableQty) {
    return next(new AppError(`Only ${availableQty} ${item.unit} available`, 400));
  }
...
exports.returnStock = catchAsync(async (req, res, next) => {
  ...
  const availableQty = item.quantity - (item.returnedQty || 0) - (item.usedQty || 0);
  
  if (returnQty > availableQty) {
    return next(new AppError(`Only ${availableQty} ${item.unit} can be returned`, 400));
  }
  ...
  await InventoryTransaction.create({
    ...
    type: 'RETURN',
    notes: notes || 'Employee returned unused stock',
    performedBy: req.user._id
  });
