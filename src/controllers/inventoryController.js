...
exports.addChemical = catchAsync(async (req, res, next) => {
  const { name, sku, category, unit, unitSystem, description, mainStock, purchasePrice, minStockLevel, manufacturer, supplier } = req.body;
...
exports.updateChemical = catchAsync(async (req, res, next) => {
  const { name, sku, category, unit, unitSystem, description, purchasePrice, minStockLevel, manufacturer, supplier } = req.body;
  ...
  if (purchasePrice !== undefined) chemical.purchasePrice = parseFloat(purchasePrice);
  if (minStockLevel !== undefined) chemical.minStockLevel = parseFloat(minStockLevel);
...
exports.deleteChemical = catchAsync(async (req, res, next) => {
  ...
  if (inventoryItems.length > 0) {
    return next(new AppError('Cannot delete chemical that is in active inventory', 400));
  }
...
exports.purchaseStock = catchAsync(async (req, res, next) => {
  ...
  if (rate <= 0) return next(new AppError('Purchase rate must be greater than 0', 400));

  const chemical = await Chemical.findById(chemicalId);
  if (!chemical) return next(new AppError('Chemical not found', 404));

  const baseQuantity = convertToBaseUnit(qty, req.body.unit || chemical.unitSystem, chemical.unitSystem);
  const totalValue = Math.round(baseQuantity * rate * 100) / 100;

  chemical.mainStock += baseQuantity;
  chemical.lastPurchaseDate = purchaseDate ? new Date(purchaseDate) : new Date();
  ...
  await InventoryTransaction.create({
    ...
    unit: chemical.unitSystem,
    purchaseRate: rate,
    transferRate: Math.round(rate * 1.10 * 100) / 100,
    unitPrice: rate,
    totalValue,
    type: 'PURCHASE',
    invoicePhotoUrl,
    ...
    supplierName,
    supplierContact,
    notes: notes || `Purchased ${qty} ${chemical.unitSystem} from ${supplierName || 'Supplier'}`,
    performedBy: req.user._id
  });
...
exports.transferToBranch = catchAsync(async (req, res, next) => {
  ...
  if (chemical.mainStock < baseQuantity) {
    return next(new AppError(`Insufficient stock. Available: ${convertFromBaseUnit(chemical.mainStock, unit || chemical.unitSystem, chemical.unitSystem)} ${displayUnit(unit || chemical.unitSystem, chemical.unitSystem)}`, 400));
  }

  const transferRate = chemical.branchPrice || Math.round(chemical.purchasePrice * 1.10 * 100) / 100;
  const totalValue = Math.round(baseQuantity * transferRate * 100) / 100;

  chemical.mainStock -= baseQuantity;
  await chemical.save();

  let branchInv = await Inventory.findOne({ chemicalId, ownerId: branchId, ownerType: 'Branch' });
  if (branchInv) {
    branchInv.quantity += baseQuantity;
    branchInv.transferRate = transferRate;
    branchInv.totalValue = Math.round(branchInv.quantity * transferRate * 100) / 100;
...
exports.transferToEmployee = catchAsync(async (req, res, next) => {
  ...
  if (req.user.role !== 'branch_admin' && req.user.role !== 'office' && req.user.role !== 'super_admin') {
    return next(new AppError('Only Branch Admin can transfer to employees', 403));
  }
  ...
  if (req.user.role === 'super_admin') {
    ...
    sourceInv = await Inventory.findOne({ chemicalId, ownerId: branchId, ownerType: 'Branch' });
  }

  if (!sourceInv || sourceInv.quantity < baseQuantity) {
    return next(new AppError(`Insufficient stock. Available: ${convertFromBaseUnit(sourceInv ? sourceInv.quantity : 0, unit || chemical.unitSystem, chemical.unitSystem)} ${displayUnit(unit || chemical.unitSystem, chemical.unitSystem)}`, 400));
  }
  ...
  let empInv = await Inventory.findOne({ chemicalId, ownerId: employeeId, ownerType: 'Employee' });
  if (empInv) {
...
exports.recordUsage = catchAsync(async (req, res, next) => {
  const { chemicalId, quantity, unit, jobId, jobName, notes, isReturned, returnTo } = req.body;
  ...
  let empInv = await Inventory.findOne({ chemicalId, ownerId: req.user._id, ownerType: 'Employee' });
  if (!empInv || empInv.quantity < baseQuantity) {
    return next(new AppError(`Insufficient wallet balance. Available: ${convertFromBaseUnit(empInv ? empInv.quantity : 0, unit || chemical.unitSystem, chemical.unitSystem)} ${displayUnit(unit || chemical.unitSystem, chemical.unitSystem)}`, 400));
  }
  ...
  if (isReturned) {
    ...
    if (returnTo === 'super_admin' || returnTo === 'HQ') {
      ...
      if (branchInv) {
        ...
        await Inventory.create({
          ...
          totalValue: Math.round(baseQuantity * chemical.branchPrice * 100) / 100,
    ...
    await InventoryTransaction.create({
      ...
      notes: notes || `Returned ${qty} ${chemical.unitSystem} to ${returnTo === 'super_admin' ? 'HQ' : 'Branch'}`,
      performedBy: req.user._id,
      returnedQuantity: baseQuantity
    });

    res.status(200).json({ success: true, message: `Returned ${qty} ${chemical.unitSystem}` });
  } else {
    empInv.quantity -= baseQuantity;
    ...
    await InventoryTransaction.create({
      ...
      jobName,
      linkedFormId: linkedForm?._id,
      notes: notes || `Used ${qty} ${chemical.unitSystem} for ${jobName || 'Job'}`,
      performedBy: req.user._id,
      returnedQuantity: 0
    });

    res.status(200).json({ success: true, message: `Usage recorded: ${qty} ${chemical.unitSystem}` });
  }
});
...
exports.getInventory = catchAsync(async (req, res, next) => {
  ...
  if (req.user.role === 'super_admin') {
    ...
    usersInBranch = await User.find({ branchId: req.user.branchId }).select('_id');
  ...
  const employeeInventories = inventory.filter(i => i.ownerType === 'Employee');
  ...
  const formatted = inventory.map(item => {
    const distributed = item.ownerType === 'Branch' 
      ? convertFromBaseUnit(distributedMap[item.chemicalId?._id?.toString()] || 0, item.unit || item.chemicalId?.unitSystem, item.chemicalId?.unitSystem)
      : 0;
      
    return {
      ...item,
      displayQuantity: convertFromBaseUnit(item.quantity, item.unit || item.chemicalId?.unitSystem, item.chemicalId?.unitSystem),
      displayUnit: displayUnit(item.unit || item.chemicalId?.unitSystem, item.chemicalId?.unitSystem),
...
exports.getTransactions = catchAsync(async (req, res, next) => {
  ...
  if (req.user.role === 'super_admin') {
    ...
    matchStage = { $or: [{ fromId: req.user._id }, { toId: req.user._id }, { performedBy: req.user._id }] };
  }
  ...
  const formatted = transactions.map(tx => ({
    ...tx,
    displayQuantity: convertFromBaseUnit(tx.quantity, tx.unit || tx.chemicalId?.unitSystem, tx.chemicalId?.unitSystem),
    displayUnit: displayUnit(tx.unit || tx.chemicalId?.unitSystem, tx.chemicalId?.unitSystem)
  }));
...
exports.getMyWallet = catchAsync(async (req, res, next) => {
  const wallet = await Inventory.find({ ownerId: req.user._id, ownerType: 'Employee' })
    .populate('chemicalId')
    .sort('-updatedAt');

  const formatted = wallet.map(item => {
    const displayQty = convertFromBaseUnit(item.quantity, item.unit || item.chemicalId?.unitSystem, item.chemicalId?.unitSystem);
    return {
      _id: item._id,
...
exports.getMyBalance = catchAsync(async (req, res, next) => {
  ...
  const payments = await Payment.find({ branchId: branch._id }).sort('-createdAt');
  
  // Get inventory stats
  const usersInBranch = await User.find({ branchId: branch._id }).select('_id');
  const userIds = usersInBranch.map(u => u._id);
  
  const branchInventory = await Inventory.find({ ownerId: branch._id, ownerType: 'Branch' });
  const employeeInventory = await Inventory.find({ ownerId: { $in: userIds }, ownerType: 'Employee' });
  
  const totalItems = branchInventory.reduce((sum, item) => sum + item.quantity, 0);
  const distributedItems = employeeInventory.reduce((sum, item) => sum + item.quantity, 0);
  
  // Get employee distribution
  const employeeDistribution = {};
  employeeInventory.forEach(inv => {
    const user = usersInBranch.find(u => u._id.toString() === inv.ownerId.toString());
    if (user) {
      const userName = user.name || 'Unknown';
      if (!employeeDistribution[userName]) {
        employeeDistribution[userName] = {};
      }
      employeeDistribution[userName][inv.chemicalId?.toString()] = (employeeDistribution[userName][inv.chemicalId?.toString()] || 0) + inv.quantity;
    }
  });
  ...
  const totalInventoryValue = inventoryTransactions.length > 0 ? inventoryTransactions[0].totalValue : 0;
...
exports.deleteInventory = catchAsync(async (req, res, next) => {
  ...
  if (!inventoryItem) return next(new AppError('Inventory allocation not found', 404));

  await Inventory.findByIdAndDelete(req.params.id);

  res.status(200).json({ success: true, message: 'Inventory allocation deleted' });
});
